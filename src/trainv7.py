...
OVERRIDES  = [
    # 例："train.epochs=36", "train.optimizer.lr=0.0025"
]
# ===============================================
...
def train_one_epoch(model, loader, optimizer, device, epoch_idx, total_epochs,
    ...
    for step, (images, targets) in enumerate(pbar, 1):
        ...
        if step % max(1, print_interval) == 0:
            # 這裡顯示的是 backbone group 的 lr（param_groups[0]）
            lr_bb  = optimizer.param_groups[0]['lr']
            lr_head = optimizer.param_groups[1]['lr'] if len(optimizer.param_groups) > 1 else lr_bb
...
def main():
    ...
    # 建立 optimizer（讀取 train.optimizer）
    O = (cfg.get("train", {}).get("optimizer", {}) or {})
    ...
    bb_lr_mult   = float(O.get("backbone_lr_mult", 0.33))
    ...
    for epoch in range(epochs):
        # 每個 epoch 起始印目前兩組 LR（方便確認 warmup / cosine）
        lr_bb  = optimizer.param_groups[0]['lr']
        lr_head = optimizer.param_groups[1]['lr'] if len(optimizer.param_groups) > 1 else lr_bb
        print(f"[LR] epoch={epoch+1} backbone={lr_bb:.3e} | head={lr_head:.3e}")
        ...
        if freeze_epochs > 0 and epoch == freeze_epochs:
            if _set_backbone_requires_grad(model, True):
                ...
                new_params = [p for p in model.parameters()
                              if p.requires_grad and (not any(p is q for g in optimizer.param_groups for q in g['params']))]
                if len(new_params):
                    optimizer.add_param_group({
                        "params": new_params,
                        "lr": base_lr * bb_lr_mult,
                        "weight_decay": wd,
                    })
                    print(f"[Backbone] added {len(new_params)} params to optimizer with lr={base_lr*bb_lr_mult:.3e}")
            else:
