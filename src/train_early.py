...
- 新增：驗證集 val_loss、Early Stopping（依 YAML early_stop.*）
...
@torch.no_grad()
def evaluate_one_epoch(model, loader, device, epoch_idx, total_epochs):
    """
    計算驗證損失（val_loss）。
    注意：torchvision detection 模型只有在 train 模式且傳入 targets 才會回傳 loss。
    因此驗證時採用 model.train() + no_grad() 的組合，避免反傳且可穩定取得 loss。
    模型中像 BN/Dropout 會維持 train 行為；但我們的偵測頭本身不依靠隨機失活，
    實務上用於比較 epoch 間趨勢是可行的（若介意，可在資料量足夠時平均更多 batch）。
    """
    if loader is None:
        return math.nan, 0.0
    ...
    total_loss, num_batches = 0.0, 0
    epoch_t0 = time.perf_counter()
    pbar = tqdm(loader, total=len(loader), ncols=120, desc=f"[Val] Epoch {epoch_idx+1}/{total_epochs}")
    ...
    for images, targets in pbar:
        ...
        num_batches += 1
        pbar.set_postfix({"val_loss": f"{total_loss/max(1,num_batches):.3f}"})

    epoch_time = time.perf_counter() - epoch_t0
    avg_loss = total_loss / max(1, num_batches)
    print(f"[Val   {epoch_idx+1}/{total_epochs}] avg_loss={avg_loss:.4f} | time={epoch_time:.1f}s")
    return avg_loss, epoch_time
...
def _is_better(curr, best, mode: str):
    if math.isnan(curr):
        return False
    mode = (mode or "min").lower()
    if best is None:
        return True
    if mode == "min":
        return curr < best - 1e-12
    elif mode == "max":
        return curr > best + 1e-12
    else:
        raise ValueError(f"early_stop.mode must be 'min' or 'max', got {mode}")
...
def main():
    ...
    if val_img_dir and val_gt and os.path.isdir(val_img_dir) and os.path.isfile(val_gt):
        val_ds = PigsDataset(
            val_img_dir,
            val_gt,
            transforms=get_transforms(train=True, max_side=cfg["augment"]["max_side"])
        ...
        print("[Info] No validation set found (data.val_img_dir / data.val_gt). Early stopping will be disabled.")
        cfg.setdefault("early_stop", {}).update({"enabled": False})
    ...
    es_mode     = str(es_cfg.get("mode", "min")).lower()  # "min" or "max"
    es_patience = int(es_cfg.get("patience", 10))
    es_save_best= bool(es_cfg.get("save_best", True))
    best_name   = es_cfg.get("best_name", f"{cfg['project']['run_name']}_best.pth")
    ...
    best_ckpt_path = os.path.join(cfg["checkpoint"]["dir"], best_name)

    # === 訓練 ===
    total_t0 = time.perf_counter()
    for epoch in range(cfg["train"]["epochs"]):
        train_avg, _ = train_one_epoch(
            model, train_loader, optimizer, device,
            epoch_idx=epoch, total_epochs=cfg["train"]["epochs"],
            grad_clip=cfg["train"]["grad_clip"]
        )

        # 計算驗證指標（目前支援 val_loss）
        val_metric = math.nan
        if val_loader is not None and es_monitor == "val_loss":
            val_metric, _ = evaluate_one_epoch(
        ...
        if es_enabled:
            curr = val_metric
            improved = _is_better(curr, best_score, es_mode)
            if improved:
                best_score = curr
                bad_epochs = 0
                if es_save_best:
                    ...
                    print(f"[Save Best] {best_ckpt_path} | {es_monitor}={curr:.6f}")
            else:
                bad_epochs += 1
                print(f"[EarlyStop] No improvement on '{es_monitor}' for {bad_epochs}/{es_patience} epochs.")

            if bad_epochs >= es_patience:
                print(f"[EarlyStop] Stop training early at epoch {epoch+1}. Best {es_monitor}={best_score:.6f}.")
                break
    ...
    print(f"[Save Final] {ckpt_path} | total_time={total_time/60:.1f} min")
