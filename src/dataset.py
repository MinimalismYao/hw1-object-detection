...
class PigsDataset(Dataset):
    ...
    傳回：image(Tensor[C,H,W] 0~1), target(dict: boxes[N,4], labels[N])
    """
    def __init__(self, img_dir, gt_txt=None, transforms=None):
        self.img_dir = img_dir
        self.transforms = transforms
        self.has_gt = gt_txt is not None
        # 影像檔名：00000001.jpg 這種
        self.img_ids = sorted([os.path.splitext(os.path.basename(p))[0]
    ...
    def __getitem__(self, idx):
        ...
        if self.has_gt:
            boxes = np.array(self.boxes_by_id.get(fid, []), dtype=np.float32)
            labels = np.ones((len(boxes),), dtype=np.int64)  # 單類別：豬=1
