...
def _make_norm(norm_type: str):
    ...
    if norm_type in ("groupnorm", "groupnorm32", "gn", "gn32"):
        def _gn(ch):  # group 數量盡量用 32，頻道不整除時退而求其次
...
def get_fasterrcnn_r101_fpn_v7(
    ...
    *,
    ...
    anc_sizes_cfg = _maybe_get(cfg or {}, "model.rpn_anchor_sizes",
                               [8, 16, 32, 64, 128, 256])
    anc_ratios_cfg = _maybe_get(cfg or {}, "model.rpn_anchor_ratios",
                                [0.5, 1.0, 2.0, 3.0])
    sizes_tuple = _as_tuple_sizes(anc_sizes_cfg)
    # 最少 5 層，與 FPN 層數對齊（多的會自動截斷）
    while len(sizes_tuple) < 5:
        sizes_tuple += (sizes_tuple[-1],)
    ratios_per_level = (tuple(float(r) for r in anc_ratios_cfg),) * len(sizes_tuple)
    ...
    # 使用 torchvision 標準 resnet + FPN；回傳層名 "0","1","2","3" 及 "pool"(P6 可選)
    # 這裡直接用 BackboneWithFPN，確保用自訂 norm
    ...
    backbone = BackboneWithFPN(body, in_channels_list, fpn_out_channels, extra_blocks=extra_blocks)
    ...
    model = FasterRCNN(
        backbone=backbone,
        num_classes=None,  # 先自行接 predictor，以便替換 head
        ...
        # ROI / Inference（先設，稍後覆蓋 bbox_coder）
    ...
    # ---- RPN 超參 ----
    model.rpn.batch_size_per_image = RPN["batch_size_per_image"]
    model.rpn.positive_fraction    = RPN["positive_fraction"]
    model.rpn.pre_nms_top_n        = dict(training=RPN["pre_nms_top_n_train"], testing=RPN["pre_nms_top_n_test"])
    model.rpn.post_nms_top_n       = dict(training=RPN["post_nms_top_n_train"], testing=RPN["post_nms_top_n_test"])
    model.rpn.nms_thresh           = RPN["nms_thresh"]
    model.rpn.foreground_iou_thresh = RPN["fg_iou_thresh"]
    model.rpn.background_iou_thresh = RPN["bg_iou_thresh"]
